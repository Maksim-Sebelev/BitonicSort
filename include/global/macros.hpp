#pragma once

//---------------------------------------------------------------------------------------------------------------

#if defined(NDEBUG) and defined(BITONICSORT_DEBUG)
#error "bad release-debug macro combo"
#endif /* defined(NDEBUG) and (defined(_DEBUG) or defined(DEBUG)) */

//---------------------------------------------------------------------------------------------------------------


#if not defined(EXIT_SUCCESS)
#define EXIT_SUCCESS 0
#endif /* not defined(EXIT_SUCCESS) */

//---------------------------------------------------------------------------------------------------------------

#if not defined(EXIT_FAILURE)
#define EXIT_FAILURE 1
#endif /* not defined(EXIT_SUCCESS) */

//---------------------------------------------------------------------------------------------------------------


#if not defined(EXIT_SUCCESS)
#define EXIT_SUCCESS 0
#endif /* not defined(EXIT_SUCCESS) */

//---------------------------------------------------------------------------------------------------------------

#if not defined(EXIT_FAILURE)
#define EXIT_FAILURE 1
#endif /* not defined(EXIT_SUCCESS) */

//---------------------------------------------------------------------------------------------------------------

#include <cassert> // for msg_assert

//---------------------------------------------------------------------------------------------------------------

#include "global/custom_console_output.hpp"

//---------------------------------------------------------------------------------------------------------------

#if defined(BITONICSORT_CXX_23_SUPPORT)
#define ON_BITONICSORT_CXX_23_SUPPORT(...) __VA_ARGS__
#define OFF_BITONICSORT_CXX_23_SUPPORT(...)
#else /* defined(BITONICSORT_CXX_23_SUPPORT) */
#define ON_BITONICSORT_CXX_23_SUPPORT(...)
#define OFF_BITONICSORT_CXX_23_SUPPORT(...) __VA_ARGS__
#endif /* defined(BITONICSORT_CXX_23_SUPPORT)*/

//---------------------------------------------------------------------------------------------------------------

#if defined(BITONICSORT_DEBUG)

//---------------------------------------------------------------------------------------------------------------

#define ON_BITONICSORT_DEBUG(...) __VA_ARGS__
#define OFF_BITONICSORT_DEBUG(...)

#define builtin_unreachable_wrapper(debug_message)                                                                     \
    msg_assert(false, "__builtin_unreachable() reaached\n" << debug_message)

#define msg_assert(bool_expression, message)                                                                           \
    do                                                                                                                 \
    {                                                                                                                  \
        if (not(bool_expression))                                                                                      \
        {                                                                                                              \
            std::cerr << RED BOLD "assertation failed:\n" RESET_CONSOLE_OUT WHITE << #bool_expression << "\n"          \
                      << __FILE__ << ":" << __LINE__ << " [" << __func__                                               \
                      << "]\n" RESET_CONSOLE_OUT RED "message:\n" BOLD << message << RESET_CONSOLE_OUT "\n";           \
            std::exit(EXIT_FAILURE);                                                                                   \
        }                                                                                                              \
    } while (0)

//---------------------------------------------------------------------------------------------------------------

#else /* defined(BITONICSORT_DEBUG) */

//---------------------------------------------------------------------------------------------------------------

#define ON_BITONICSORT_DEBUG(...)
#define OFF_BITONICSORT_DEBUG(...) __VA_ARGS__

#define builtin_unreachable_wrapper(debug_message) __builtin_unreachable()

#define msg_assert(bool_expression, message)

//---------------------------------------------------------------------------------------------------------------

#endif /* defined(BITONICSORT_DEBUG) */

//---------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------

#define msg_abort(bool_expression, message)                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        if (not(bool_expression))                                                                                      \
        {                                                                                                              \
            std::cerr << RED BOLD                                                                                      \
                "run program failed: " ON_BITONICSORT_DEBUG("\n")                                                      \
                    RESET_CONSOLE_OUT WHITE ON_BITONICSORT_DEBUG(<< #bool_expression << "\n"                           \
                                                                 << __FILE__ << ":" << __LINE__ << " [" << __func__    \
                                                                 << "]\n" RED "message:\n") /* ON_BITONICSORT_DEBUG */ \
                BOLD << message                                                                                        \
                      << RESET_CONSOLE_OUT "\n";                                                                       \
            std::exit(EXIT_FAILURE);                                                                                   \
        }                                                                                                              \
    } while (0)

//---------------------------------------------------------------------------------------------------------------

#define msg_warning(bool_expression, message)                                                                          \
    do                                                                                                                 \
    {                                                                                                                  \
        if (not(bool_expression))                                                                                      \
        {                                                                                                              \
            std::cerr << YELLOW BOLD                                                                                   \
                "warning: " ON_BITONICSORT_DEBUG("\n")                                                                 \
                    RESET_CONSOLE_OUT WHITE ON_BITONICSORT_DEBUG(<< #bool_expression << "\n"                           \
                                                                 << __FILE__ << ":" << __LINE__ << " [" << __func__    \
                                                                 << "]\n" YELLOW                                       \
                                                                    "message:\n") /* ON_BITONICSORT_DEBUG */           \
                BOLD << message                                                                                        \
                      << RESET_CONSOLE_OUT "\n";                                                                       \
        }                                                                                                              \
    } while (0)

//---------------------------------------------------------------------------------------------------------------

#if defined(BITONICSORT_TIME_MEASURE)
#define ON_TIME(...) __VA_ARGS__
#else /* defined(TIME) */
#define ON_TIME(...) 
#endif /* defined(TIME) */

//---------------------------------------------------------------------------------------------------------------

#if defined(LOG)
#define ON_LOGGER(...) __VA_ARGS__
#else /* defined(LOG) */
#define ON_LOGGER(...)
#endif /* defined(LOG) */

//---------------------------------------------------------------------------------------------------------------

#if defined(SHOW_EXCEPTION_STACKTRACE)
#define ON_STACKTRACE(...) __VA_ARGS__
#else /* defined(SHOW_EXCEPTION_STACKTRACE) */
#define ON_STACKTRACE(...)
#endif /* defined(SHOW_EXCEPTION_STACKTRACE) */

//---------------------------------------------------------------------------------------------------------------

#define STRINGIFY(X) #X

//---------------------------------------------------------------------------------------------------------------

#define dbgs \
if constexpr (not DEBUG_STREAM) {} \
else \
    std::cout 

//---------------------------------------------------------------------------------------------------------------
